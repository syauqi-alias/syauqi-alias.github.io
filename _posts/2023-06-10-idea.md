...
Purely based on the CM5 IO carrier board. Opt for a higher clearance M.2 slot so that I can place the step down converter under NVMe drive, save some space. *Note to self; make sure the drive is only populated in one side*.

For a bike-computer, this prolly don't make sense at all. Literally designing a PDA device. Bet it wont last even an hour of operation. Heh, at least we learn by doing mistake don't we (。_。) [flashing through me mind all the fail project :p]
